...
class DsWikiProcessor:
    """
    Encapsulates the entire processing pipeline for the DSWiki data source.
    """

    def __init__(self, root_directory, output_directory, chroma_persist_directory):
        """
        Initializes the processor with necessary directory paths.
        """
    ...
    def process(self):
        ...
        if final_chunks:
            # Step 2: Export chunks to files for inspection
    ...
    @staticmethod
    def _convert_diagram_to_text(content):
        # Pass 1: Handle properly wrapped ```mermaid blocks
        processed_content = re.sub(
            r"```mermaid(.*?)```",
            lambda m: DsWikiProcessor._parse_mermaid_logic(m.group(1)),
            content,
            flags=re.DOTALL,
        )
        ...
        if (
            is_likely_naked_diagram
            and "The process is as follows" not in processed_content
        ):
            # It's a naked diagram, so process the whole string
            processed_content = DsWikiProcessor._parse_mermaid_logic(processed_content)
    ...
    @staticmethod
    def _clean_chunk_content(content):
        # Check if the content looks like it contains any part of a mermaid diagram
        if (
            "-->" in content or "&gt;" in content or "classDef" in content
        ) and re.search(r"\w+\(\[", content):
            text = DsWikiProcessor._parse_mermaid_logic(content)
        else:
            text = content
    ...
    @staticmethod
    def _load_and_preprocess_file(file_path):
        """
    ...
    @staticmethod
    def _split_faq_style(content, metadata):
    ...
    def _process_all_markdown_files(self, root_directory):
        """
        ...
        for dirpath, _, filenames in os.walk(root_directory):
            for filename in filenames:
                if filename.endswith(".md"):
                    ...
                    chunks = self._chunk_document(file_path, root_directory)
    ...
    @staticmethod
    def _export_chunks(chunks, output_dir="."):
        """
